const TB08031Sjs = (function () {
  ...
  function loadSelectBoxContents() {
    ...
    p001List = selectBoxList_TB08031S.filter(
      (item) => item.cmnsGrpCd === "P001"
    );
    b007List = selectBoxList_TB08031S.filter(
      (item) => item.cmnsGrpCd === "B007"
    );
    i051List = selectBoxList_TB08031S.filter(
      (item) => item.cmnsGrpCd === "I051"
    );
    i038List = selectBoxList_TB08031S.filter(
      (item) => item.cmnsGrpCd === "I038"
    );
    e016List = selectBoxList_TB08031S.filter(
      (item) => item.cmnsGrpCd === "E016"
    );
  ...
  function setGrid_TB08031S() {
    ...
    udwrtPaiBzscalInfoInstance = $("#TB08031S_udwrtPaiBzscalInfo").pqGrid(
      "instance"
    );
    ...
    function businessFunction2() {
      ...
      $.ajax({
        ...
        success: function (data) {
          ...
          var excBlce = isEmpty(bssBscInfo.excBlce)
            ? 0
            : addComma(bssBscInfo.excBlce);
          var expDt = isEmpty(bssBscInfo.expDt)
            ? ""
            : formatDate(bssBscInfo.expDt);
          var rgstDt = isEmpty(bssBscInfo.rgstDt)
            ? ""
            : formatDate(bssBscInfo.rgstDt);
          ...
          $('div[data-menuid="/TB08031S"] #UPLOAD_AddFile').attr(
            "disabled",
            false
          );
          $('div[data-menuid="/TB08031S"] #UPLOAD_DelFiles').attr(
            "disabled",
            false
          );
  ...
  function gridInfoSett(rowData, instncNm) {
    ...
    if (instncNm === "bsnsPartInfoInstance") {
      //사업참가자정보
    ...
    } else if (instncNm === "bondProtInfoInstance") {
      //채권보전주요약정
    ...
    } else if (instncNm === "cchInfoInstance") {
      //조건변경이력
    ...
    } else if (instncNm === "stlnInfoInstance") {
      //대주단정보
    ...
    } else if (instncNm === "ernInfoInstance") {
      //수익자정보
    ...
    } else if (instncNm === "busiInfoInstance") {
      //관련사업정보
    ...
    } else if (instncNm === "invstBzscalInstance") {
      //투자기업목록
    ...
    } else if (instncNm === "asstWrkngInfoInstance") {
      //자산운용사정보
    ...
    } else if (instncNm === "admsAsstInfoInstance") {
      //편입자산정보
  ...
  function paramSett() {
    ...
    if (invFnnMngmBusiDcd === "01") {
      ...
      var rdmpCpltYn = $("input[name=TB08031S_rdmpCpltYn]:checked").val(); //상환완료여부
      var busiLcsiCpltYn = $(
      ...
      var brwrSpcYn = $("input[name=rlesSpcYN]:checked").val(); //차주SCP여부
  ...
  function bondProtInfoBtnSave(mode) {
    ...
    function businessFunction(mode) {
      ...
      var fnnrCtrcMttrTrgtYn = $(
        "input[name=TB08031S_bondPfrmYN]:checked"
      ).val(); // 재무약정사항대상여부
  ...
  function getReltDealInfo(dealNo) {
    ...
    $.ajax({
      ...
      success: function (data) {
        ...
        console.log("datacheck:::::::", paramData);
  ...
  function admsAsstInfoBtnSave(mode) {
    ...
    function businessFunction(mode) {
      ...
      var admsAsstGrntErnRt = $("#TB08031S_invstWeight")
        .val()
        .replaceAll(",", ""); //편입자산보장수익율
  ...
  function monthDiff_TB08031S(btnId) {
    ...
    if (isEmpty($(strtDtId).val()) || isEmpty($(endDtId).val())) {
      ...
      var monthDiffVal = monthDiff(
        $(strtDtId).val().replaceAll("-", ""),
        $(endDtId).val().replaceAll("-", "")
      );
