const TB08090Sjs = (function () {
  ...
  function setGrid_TB08090S() {
          ...
          listener: function () {
            ...
            var transformedData = data.map((row) => {
              ...
              var option = options.find(
                (opt) => opt.cdValue == row.astsQtyDvdCd
              );
    ...
    $("#TB08090S_gridList .pq-toolbar .ui-button").attr(
      "id",
      "download-file-TB08090S"
    ); //엑셀 다운로드 버튼 id부여
  ...
  function asstSnnSave() {
    ...
    var asstSnnInfoLgth = $("#TB08090S_gridList").pqGrid(
      "option",
      "dataModel.data"
    ).length; //건전성 및 충당금 산출 그리드

    if (isEmpty($("#TB08090_stdrDt").val())) {
      //기준일자 미입력 시
      ...
      for (var i = 0; i < asstSnnInfoLgth; i++) {
        var rowData = $("#TB08090S_gridList").pqGrid("getRowData", {
          rowIndx: i,
        });
    ...
    function businessFunction() {
      ...
      asstSnnInfo.forEach(function (item) {
        ...
        //잔여만기년수 계산
        if (expDt == 0 || expDt == "" || isEmpty(expDt)) {
          //만기일자 Null 인 경우
          rmnExpYnum = 0; //잔여만기년수
        } else {
          rmnExpYnum =
            Number(expDt.substring(0, 4)) - Number(stdrDt.substring(0, 4)); //잔여만기년수
        }
        //alert(rmnExpYnum);
        ...
        var dtoInfo = {
          ...
          prdtCd: prdtCd,
          excSn: excSn,
          excDt: excDt.replaceAll("-", ""),
          expDt: isEmpty(expDt) ? "0" : expDt.replaceAll("-", ""),
          indvEvlAprnAmt: isEmpty(indvEvlAprnAmt)
            ? "0"
            : indvEvlAprnAmt.replaceAll(",", ""),
          acbkAmt: acbkAmt,
  ...
  function addExcelRows(rows) {
    ...
    rows.forEach(function (row) {
      ...
      $("#TB08090S_gridList").pqGrid("addRow", {
        rowData: newRow,
        checkEditable: false,
      });
    });
