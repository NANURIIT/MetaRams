...
function TB06011P_srchPrdt(menuId) {
   ...
   */
  $(
    `div[data-menuid="${menuId}"] span.input-group-append > button[onclick*="callTB06011P"]:not([disabled])`
  )
    .closest("span.input-group-append")
    .prev("input[id*='_prdtCd']")
...
async function getPrdtCdList() {
  ...
  if (
    $("#TB06011P_prefix").val() == "TB07020S" ||
    $("#TB06011P_prefix").val() == "TB07020S_input" ||
    $("#TB06011P_prefix").val() == "TB07040S" ||
    $("#TB06011P_prefix").val() == "TB07040S_input"
  ) {
    trDvsn = "T";
  }

  if (
    $("#TB06011P_prefix").val() == "TB07010S" ||
    $("#TB06011P_prefix").val() == "TB07030S" ||
    $("#TB06011P_prefix").val() == "TB07050S"
  ) {
    trDvsn = "L";
  }
  ...
  if (
    ...
    $("#TB06011P_prefix").val() == "TB07060S" ||
    $("#TB06011P_prefix").val() == "TB07160S"
  ) {
    trDvsn = "S";
  }
  ...
  if (
    $("#TB06011P_prefix").val() == "TB07150S" ||
    $("#TB06011P_prefix").val() == "TB07080S"
  ) {
    trDvsn = "TB07150S";
  }
  ...
  await $.ajax({
    ...
    success: function (data) {
      if (
        $(`div[id='modal-TB06011P']`).css("display") === "none" &&
        data.length === 1
      ) {
        ...
        setTimeout(
          () => $("#TB06011P_prdtCdList").pqGrid("instance").setData(data),
          400
        );
      }
    },
...
function TB06011P_setPrdtInfo(e) {
  ...
  var pageEprzCrdlCtrcAmt =
    "#" + $("#TB06011P_prefix").val() + "_eprzCrdlCtrcAmt";
  var pageKrwTrslExcBlce =
    "#" + $("#TB06011P_prefix").val() + "_krwTrslExcBlce";
  ...
  var pageeprzCrdlApvlAmt =
    "#" + $("#TB06011P_prefix").val() + "_eprzCrdlApvlAmt";
  ...
  if (prefix == "TB07020S_input" || prefix == "TB07040S_input") {
    pageTrCrryCd =
      "#" + $("#TB06011P_prefix").val().replace("_input", "") + "_I027";
    pageStdrExrt =
      "#" + $("#TB06011P_prefix").val().replace("_input", "") + "_stdrExrt";
    pageWholIssuShqt =
      "#" + $("#TB06011P_prefix").val().replace("_input", "") + "_wholIssuShqt";
    pageHldgShqt =
      "#" + $("#TB06011P_prefix").val().replace("_input", "") + "_hldgShqt";
    pageTrdeExrt =
      "#" + $("#TB06011P_prefix").val().replace("_input", "") + "_trdeExrt";
    pageHoldPrpsDcd =
      "#" + $("#TB06011P_prefix").val().replace("_input", "") + "_H002";
    pageTrQnt =
      "#" + $("#TB06011P_prefix").val().replace("_input", "") + "_trQnt";
    pageTrUnpr =
      "#" + $("#TB06011P_prefix").val().replace("_input", "") + "_trUnpr";
    pageQotaRt =
      "#" + $("#TB06011P_prefix").val().replace("_input", "") + "_qotaRt";
    pageEprzCrdlCtrcAmt =
      "#" +
      $("#TB06011P_prefix").val().replace("_input", "") +
      "_eprzCrdlCtrcAmt";
    pageKrwTrslExcBlce =
      "#" +
      $("#TB06011P_prefix").val().replace("_input", "") +
      "_krwTrslExcBlce";
    pageTrslAmt =
      "#" + $("#TB06011P_prefix").val().replace("_input", "") + "_trslAmt";
    pageKrwTrslExcAmt =
      "#" +
      $("#TB06011P_prefix").val().replace("_input", "") +
      "_krwTrslExcAmt";
    pageAvrUnpr =
      "#" + $("#TB06011P_prefix").val().replace("_input", "") + "_avrUnpr";
    // pageEvlPflsAmt = '#' + $('#TB06011P_prefix').val().replace('_input', '') + '_evlPflsAmt';
    pageTradPflsAmt =
      "#" + $("#TB06011P_prefix").val().replace("_input", "") + "_tradPflsAmt";
  } else {
    var pageTrCrryCd = "#" + $("#TB06011P_prefix").val() + "_trCrryCd";
    var pageStdrExrt = "#" + $("#TB06011P_prefix").val() + "_stdrExrt";
  }
  ...
  $(pageWholIssuShqt).val(
    isEmpty(e.wholIssuShqt) ? "0" : addComma(e.wholIssuShqt)
  );
  ...
  var qotaRt = isEmpty(e.hldgShqt)
    ? "0"
    : ((e.hldgShqt / e.wholIssuShqt) * 100).toFixed(2);
  //alert(qotaRt);
  if (!isFinite(qotaRt)) {
    $(pageQotaRt).val("0");
  } else {
  ...
  $(pageEprzCrdlCtrcAmt).val(
    isEmpty(e.eprzCrdlCtrcAmt) ? "0" : addComma(e.eprzCrdlCtrcAmt)
  );
  $(pageKrwTrslExcBlce).val(
    isEmpty(e.krwTrslExcBlce) ? "0" : addComma(e.krwTrslExcBlce)
  );
  $(pageTrslAmt).val(0);
  $(pageKrwTrslExcAmt).val(
    isEmpty(e.krwTrslExcAmt) ? "0" : addComma(e.krwTrslExcAmt)
  );
  $(pageAvrUnpr).val(isEmpty(e.avrUnpr) ? "0" : addComma(e.avrUnpr));
  $(pageEvlPflsAmt).val(isEmpty(e.evlPflsAmt) ? "0" : addComma(e.evlPflsAmt));
  $(pageTradPflsAmt).val(
    isEmpty(e.tradPflsAmt) ? "0" : addComma(e.tradPflsAmt)
  );
  ...
  if (prefix === "TB07100S_grid") {
    const rowId = $('#TB07100S_grd_thdtTrDtls').find(`div[role=row][aria-rowIndex=${prdtSn + 1}]`).attr('id').split("-")
    ...
    $("#TB07100S_grd_thdtTrDtls").find(`#pq-body-cell-${rowId[3]}-${prdtSn}-10-right div`).html(e.prdtCd);
    $("#TB07100S_grd_thdtTrDtls").find(`#pq-body-cell-${rowId[3]}-${prdtSn}-12-right div`).html(e.ortnFndCd);
  }

  if (prefix === "TB07110S_grid") {
    const rowId = $('#TB07110S_grd_basic').find(`div[role=row][aria-rowIndex=${prdtSn + 1}]`).attr('id').split("-")
    ...
    $("#TB07110S_grd_basic").find(`#pq-body-cell-${rowId[3]}-${prdtSn}-6-right div`).html(e.prdtCd);
    $("#TB07110S_grd_basic").find(`#pq-body-cell-${rowId[3]}-${prdtSn}-8-right div`).html(e.ortnFndCd);
  }
