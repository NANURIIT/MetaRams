...
const TB06010Sjs = (function () {
  ...
  function SetEarlyRepayFeeBtn() {
    ...
    $('input[type="radio"][name="TB06010S_earlyRepayYn"]').on(
      "click",
      function () {
        var chkValue = $(
          'input[type="radio"][name="TB06010S_earlyRepayYn"]:checked'
        ).val();
  ...
  function getCnfrncDealInfo(ibDealNo, riskInspctCcd, lstCCaseCcd, prdtCd) {
    ...
    $.ajax({
      ...
      success: function (data) {
        ...
        psblRsltnYn = isEmpty(dealDetail.psblRsltnYn)
          ? "N"
          : dealDetail.psblRsltnYn;
        ...
        $("#TB06010S_apvlAmt").val(
          Number(dealDetail.sumApvlAmt).toLocaleString()
        ); // 승인금액
        ...
        $("#TB06010S_ardyBzepNo").val(
          handleNullData(checkBrnAcno(dealDetail.trOthrDscmNo))
        ); // 거래상대방식별번호
        ...
        $("#TB06010S_I012")
          .val(dealDetail.dmsCrdtGrdDcd)
          .prop("selected", true); // 내부신용등급(신용등급코드)
        $("#TB06010S_crdtInqDt").val(formatDate(dealDetail.crdtInqDt)); // 신용조회일

        /** 자산분류 정보 */

        $("#TB06010S_E022")
          .val(dealDetail.prdtLclsCd)
          .prop("selected", true)
          .change(); // 투자상품대분류
        $("#TB06010S_E023")
          .val(dealDetail.prdtMdclCd)
          .prop("selected", true)
          .change(); // 투자상품중분류
        $("#TB06010S_P004")
          .val(dealDetail.prdtClsfCd)
          .prop("selected", true)
          .change(); // 투자상품소분류
        $("#TB06010S_I002").val(dealDetail.ibPrdtClsfCd).prop("selected", true); // 투자상품상세분류
        $(":radio[name='TB06010S_altnInvYn']").radioSelect(
          dealDetail.altnInvYn
        ); // 대체투자여부
        ...
        $(":radio[name='TB06010S_rlesFnnYn']").radioSelect(
          dealDetail.rlesFnnYn
        ); // 부동산금융여부
        $("#TB06010S_R017")
          .val(dealDetail.rlesFnnDetlDcd)
          .prop("selected", true); // 부동산금융구분코드
        ...
        $("#TB06010S_sppiSfcYn")
          .val(dealDetail.sppiSfcYn)
          .prop("selected", true); // SPPI만족여부
        ...
        $(":radio[name='TB06010S_rgstCbndYn']").radioSelect(
          dealDetail.rgstCbndYn
        ); // 등록사채여부

        /** 금융조건 정보 */

        $("#TB06010S_rcgAmt").val(
          Number(handleNullData(dealDetail.apvlAmt)).toLocaleString()
        ); // 종목승인금액
        $("#TB06010S_I027").val(dealDetail.trCrryCd).prop("selected", true); // 투자통화코드
        $("#TB06010S_E010")
          .val(dealDetail.eprzCrdlIndvLmtDcd)
          .prop("selected", true); // 한도구분코드
        $("#TB06010S_ctrcPrdDcd")
          .val(dealDetail.ctrcPrdDcd)
          .prop("selected", true); // 약정기간구분코드
        $("#TB06010S_ctrcPrdMnum").val(dealDetail.ctrcPrdMnum); // 약정기간

        $("#TB06010S_T002").val(dealDetail.thcoRlDcd).prop("selected", true); // 당사역할구분코드

        $(":radio[name='TB06010S_sglLoanYn']").radioSelect(
          dealDetail.sglLoanYn
        ); // 단독대출여부
        $(":radio[name='TB06010S_mrtgStupYn']").radioSelect(
          dealDetail.mrtgStupYn
        ); // 담보제공여부
        $(":radio[name='TB06010S_sdnTrgtYn']").radioSelect(
          dealDetail.sdnTrgtYn
        ); // 셀다운대상여부
        ...
        if (isEmpty($("#TB06010S_res_prdtCd").val())) {
          ...
          $('div[data-menuid="/TB06010S"] #UPLOAD_AddFile').attr(
            "disabled",
            false
          );
          $('div[data-menuid="/TB06010S"] #UPLOAD_DelFiles').attr(
            "disabled",
            false
          );
        }
  ...
  function getIBIMS202BDTOInfo(prdtCd) {
    ...
    $.ajax({
      ...
      success: function (data) {
        ...
        $("#TB06010S_E020")
          .val(repayInterestInfo.paiRdmpDcd)
          .prop("selected", true); // 기업여신원리금상환구분코드
        ...
        $("#TB06010S_istmDtmRdmpAmt").val(
          Number(
            handleNullData(repayInterestInfo.istmDtmRdmpAmt)
          ).toLocaleString()
        ); // 만기상환금액(할부일시상환금액)

        if ("Y" == repayInterestInfo.earlyRepayYn) {
          $(
            "input:radio[name = 'TB06010S_earlyRepayYn']:input[value='Y']"
          ).attr("checked", true); // 중도상환여부
        } else {
          $(
            "input:radio[name = 'TB06010S_earlyRepayYn']:input[value='N']"
          ).attr("checked", true); // 중도상환여부
        }
        ...
        $("#TB06010S_S003")
          .val(repayInterestInfo.aictStdrIntrtKndCd)
          .prop("selected", true); // 변동금리기준금리종류코드(올인코스트기준금리종류코드)
        ...
        $("#TB06010S_intrCngeFrqcMnum").val(
          repayInterestInfo.intrtCngeFrqcMnum
        ); // 금리변동주기개월수
        $("#TB06010S_E019")
          .val(repayInterestInfo.ovduIntrRtDcd)
          .prop("selected", true); // 연체이자율구분코드(기업여신연체이자율구분코드)
        ...
        $("#TB06010S_E011")
          .val(repayInterestInfo.intrBnaoDcd)
          .prop("selected", true); // 이자선후취구분코드(기업여신이자선후취구분코드)
        $("#TB06010S_E013")
          .val(repayInterestInfo.intrDnumClcMthCd)
          .prop("selected", true); // 이자일수계산방법코드(기업여신이자일수계산방법코드)
        $("#TB06010S_E024")
          .val(repayInterestInfo.tfdLyAplyDcd)
          .prop("selected", true); // 이자초일말일적용(기업여신초일말일적용구분코드)
        $("#TB06010S_I019")
          .val(repayInterestInfo.intrSnnoPrcsDcd)
          .prop("selected", true); // 이자단수법구분(기업여신이자단수처리구분코드)
        $("#TB06010S_H001")
          .val(repayInterestInfo.hldyPrcsDcd)
          .prop("selected", true); // 이자휴일처리구분(기업여신휴일처리구분코드)
        $("#TB06010S_I016")
          .val(repayInterestInfo.intrClcEndDeDcd)
          .prop("selected", true); // 이자계산종료일구분(기업여신이자계산종료일구분코드)
      },
    }); /* end of ajax*/
  }
  ...
  function registIBIMS202BDTOInfo() {
    ...
    if (isEmpty(prdtCd)) {
      ...
      aictStdrIntrtKndCd != "0" &&
      (isEmpty(intrCngeFrqcMnum) || intrCngeFrqcMnum === "0")
    ) {
      Swal.fire({
        icon: "warning",
        title: "Warning!",
    ...
    var paramData = {
      ...
      istmDtmRdmpAmt:
        replaceAll($("#TB06010S_istmDtmRdmpAmt").val(), ",", "") / 1, // 만기상환금액(할부일시상환금액)
  ...
  function checkParam() {
    ...
    if ($("#TB06010S_E022").val() != 92) {
      ...
      if (isEmpty($("input[name=TB06010S_rlesFnnYn]:checked").val())) {
        ...
        $("input[name=TB06010S_rlesFnnYn]:checked").val() == "Y" &&
        $("#TB06010S_R017").val() == ""
      ) {
        option.text = "부동산금융여부를 선택해주세요.";
        openPopup(option);
      ...
      if (isEmpty($("input[name=TB06010S_socYn]:checked").val())) {
        ...
        $("input[name=TB06010S_socYn]:checked").val() == "Y" &&
        $("#TB06010S_S002").val() == ""
      ) {
        option.text = "SOC여부를 선택해주세요.";
        openPopup(option);
      ...
      if (
        isEmpty($("#TB06010S_dprtCd").val()) &&
        isEmpty($("#TB06010S_empNo").val())
      ) {
  ...
  function getIBIMS208BDTOInfo(prdtCd) {
    ...
    if (
      isEmpty($("#TB06010S_res_prdtCd").val()) &&
      $("#TB06010S_E022").val() !== "92"
    ) {
      $("#registApvlCnd").attr("disabled", false);
    }
  ...
  function addRowItrRelr() {
    ...
    let bbb = $("#TB06010S_itrRelrInfo").pqGrid("addRow", {
      rowData: newRow,
      checkEditable: false,
    });
  ...
  function delRowItrRelr() {
    var gridData = $("#TB06010S_itrRelrInfo").pqGrid(
      "option",
      "dataModel.data"
    );
    var rowData;
  ...
  function assetAmt() {
    ...
    $("#TB06010S_crryAmt").keyup(function (event) {
      if (
        ...
        // 1. 숫자입력 체크
        var input1 = $("#TB06010S_aplyExrt").val();
        if (input1 != "") {
          // 2-1. 적용환율 값이 있을경우
          var input2 = $("#TB06010S_crryAmt").val().replace(/,/g, ""); // 콤마 제거
          $("#TB06010S_crevAmt").val(
            Math.floor(input1 * input2).toLocaleString("ko-KR")
          );
        } else {
          // 2-2. 적용환율 값이 없을경우
    ...
    // 적용환율
    $("#TB06010S_aplyExrt").keyup(function (event) {
      if (
        (event.key >= 0 && event.key <= 9) ||
        event.key === "Backspace" ||
        event.key === "Delete"
        ...
        if (input1 != "") {
          // 2. 값이 있으면 계산
          var input2 = $("#TB06010S_aplyExrt").val();
          $("#TB06010S_crevAmt").val(
            Math.floor(input1 * input2).toLocaleString("ko-KR")
          );
        }
  ...
  function assetInfoDetails(e) {
    ...
    $("#TB06010S_aplyExrt").val(aplyExrt); // 환율
  ...
  function tab5BtnSave() {
    ...
    var bssAsstIsuCrno = $("#TB06010S_bsc_corpRgstNo")
      .val()
    ...
    var aplyExrt = $("#TB06010S_aplyExrt").val(); // 환율
  ...
  function getDealInfoFromWF() {
    if (
      sessionStorage.getItem("isFromWF") ||
      sessionStorage.getItem("isFromApvl")
    ) {
