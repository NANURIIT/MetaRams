...
const TB09012Sjs = (function () {
	...
	function setArrPqGridObj() {
		...
		colCpdgList = [
			{ title: "법인채무보증집중일련번호", dataType: "string", dataIndx: "cpdgFcsSn", align: "center", width: "170", filter: { crules: [{ condition: "range" }] }, },
			{
				...
				editor: {
					type: "select", valueIndx: "cdValue", labelIndx: "cdName", options: K002,
				},
				render: function (ui) {
					var options = K002;
			...
			},
			{
				...
				filter: { crules: [{ condition: "range" }] },
				editor: {
					type: "select", valueIndx: "cdValue", labelIndx: "cdName", options: K015,
				},
				render: function (ui) {
					var options = K015;
					var option = options.find(opt => opt.cdValue == ui.cellData);
		...
		colTransList = [
			...
			{ title: "발송시간", dataType: "string", dataIndx: "sendTm", align: "left", halign: "center", format: "HH:mm:ss", filter: { crules: [{ condition: "range" }] }, width: 150 },
	...
	function exDnLoad(grd) {
		...
		if (grd == 'gridCpdgList') {
			...
			fileName = "전문송신내역_법인채무보증_채무인수약정.xlsx";
		} else if (grd == 'gridErrList') {
			fileName = "오류통보내역_법인채무보증_채무인수약정.xlsx";
		}
