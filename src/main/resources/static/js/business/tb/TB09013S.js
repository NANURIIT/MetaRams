 ...
 * 2025.07.08 ejchoi
 */
const TB09013Sjs = (function() {

	let arrPqGrid_lmtFcssList;
	...
	function loadSelectBoxContents() {
		...
		selectBox = getSelectBoxList("TB09013S", item, false);
	...
	function rendorGrid() {
		...
		col_lmtFcssList = [
			{ title: "한도정보집중일련번호", dataType: "string", dataIndx: "lmtFcsSn", align: "left", halign: "center", filter: { crules: [{ condition: "range" }] }, width: 150 },
			...
			{ title: "한도정보과목코드(K010)", dataType: "string", dataIndx: "lmtInfSbjCd", align: "left", halign: "center", filter: { crules: [{ condition: "range" }] }, width: 150, editor: { type: 'select', valueIndx: 'cdValue', labelIndx: 'cdName', options: grdSelect.K010 }, render: function(ui) { var options = grdSelect.K010; var option = options.find(opt => opt.cdValue == ui.cellData); return option ? option.cdValue + '.' + option.cdName : ui.cellData; } },
			{ title: "회전한도금액(천원)", dataType: "string", dataIndx: "revLimitAmt", align: "left", halign: "center", filter: { crules: [{ condition: "range" }] }, width: 150 },
			{ title: "대출잔액(천원)", dataType: "string", dataIndx: "", align: "left", halign: "center", filter: { crules: [{ condition: "range" }] }, width: 150 },
		...
		arrPqGrid_lmtFcssList = $("#TB09013S_lmtFcssList").pqGrid("instance");
		arrPqGrid_msgTranList = $("#TB09013S_msgTranList").pqGrid("instance");
		arrPqGrid_errDpchList = $("#TB09013S_errDpchList").pqGrid("instance");
	...
	function rtTB09013S() {
		...
		arrPqGrid_lmtFcssList.setData([]);
		arrPqGrid_lmtFcssList.refreshDataAndView();
	...
	function shTB09013S() {
		...
		if (isEmpty($("#TB09013S_stdrDt").val())) {
	...
	// 한도정보집중내역 조회
	function lmtFcssList() {
		var paramData = {
			stdrDt: $("#TB09013S_stdrDt").val().replaceAll("-", ""),
		};

		$.ajax({
			type: "GET",
			url: "/TB09013S/getLmtFcssList",
			data: paramData,
			dataType: "json",
			success: function(data) {
				...
				arrPqGrid_lmtFcssList.setData(data);
				msgTranList();
				errDpchList();
			},
			error: function() {
	...
	// 한도정보집중내역 저장
	function saveLmtFcssList(ibims752bVOList) {
		var paramDTO = { "stdrDt": $('#TB09013S_stdrDt').val().replaceAll('-', ''), "ibims752bVOList": ibims752bVOList, "scrnDcd": "TB09013S" };

		$.ajax({
			type: "POST",
			url: "/TB09013S/saveLmtFcssList",
	...
	// 전문송신내역 저장
	function saveMsgTranList(ibims752bVOList) {
		var paramDTO = { "stdrDt": $('#TB09013S_stdrDt').val().replaceAll('-', ''), "ibims752bVOList": ibims752bVOList, "scrnDcd": "TB09013S" };

		$.ajax({
			type: "POST",
			url: "/TB09013S/saveMsgTranList",
			data: JSON.stringify(paramDTO),
			contentType: "application/json; charset=utf-8",
			dataType: "json",
			success: function() {
	...
	// 오류통보내역 저장
	function saveErrDpchList(ibims752bVOList) {
		var paramDTO = { "stdrDt": $('#TB09013S_stdrDt').val().replaceAll('-', ''), "ibims752bVOList": ibims752bVOList, "scrnDcd": "TB09013S" };

		$.ajax({
			type: "POST",
			url: "/TB09013S/saveErrDpchList",
			data: JSON.stringify(paramDTO),
			contentType: "application/json; charset=utf-8",
			dataType: "json",
			success: function() {
	...
	function exDnLoad(grd) {
		let blob = $("#TB09013S_" + grd).pqGrid("instance").exportExcel({});
		let fileName;

		if (grd == 'lmtFcssList') {
			fileName = "한도정보집중내역.xlsx";
	...
	function exUpLoad(grd) {
		...
		let gcnt = 0;
		if (grd == 'lmtFcssList') {
			gcnt = arrPqGrid_lmtFcssList.pdata.length;
		} else if (grd == 'msgTranList') {
	...
	// 엑셀업로드 버튼 이벤트 매핑
	$("#upload-file-lmtFcssList").change(function() {
		readExcelFile('lmtFcssList');
		resetFileInput($('#upload-file-lmtFcssList'));
	});
	...
	// 한도정보집중내역 행추가
	function addExcelRows_lmtFcssList(rows) {

		$("#TB09013S_lmtFcssList").pqGrid("setData", []);
		var ibims752bVOList = [];
		var seq = 0;

		rows.forEach(function(row) {
			var lmtFcsSn = row['한도정보집중일련번호'];
			var errDcd = row['오류코드(K002)'];
			var stdrDt = $("#TB09013S_stdrDt").val().replaceAll("-", "");
			...
			var eprzNm = row['상호명'];
			var rprsNm = row['대표자명'];
			var lmtInfSbjCd = row['한도정보과목코드(K010)'];
			var revLimitAmt = row['회전한도금액(천원)'];
			var loanBlce = row['대출잔액(천원)'];
			...
			var newRow = {
				...
				eprzNm: eprzNm,
				rprsNm: rprsNm,
				lmtInfSbjCd: lmtInfSbjCd,
				revLimitAmt: revLimitAmt,
				loanBlce: loanBlce,
			}

			ibims752bVOList.push(newRow);
			$("#TB09013S_lmtFcssList").pqGrid("addRow", { rowData: newRow, checkEditable: false });
		});

		$("#TB09013S_lmtFcssList").pqGrid("refreshDataAndView");
		saveLmtFcssList(ibims752bVOList);
	}

	// 전문송신내역 행추가
	function addExcelRows_msgTranList(rows) {

		$("#TB09013S_msgTranList").pqGrid("setData", []);
		var ibims752bVOList = [];
		var sn = 0;
		...
		rows.forEach(function(row) {
			...
			ibims752bVOList.push(newRow);
			$("#TB09013S_msgTranList").pqGrid("addRow", { rowData: newRow, checkEditable: false });

		});

		$("#TB09013S_msgTranList").pqGrid("refreshDataAndView");
		saveMsgTranList(ibims752bVOList);
	}

	// 오류통보내역 행추가
	function addExcelRows_errDpchList(rows) {

		$("#TB09013S_errDpchList").pqGrid("setData", []);
		var ibims752bVOList = [];
		var seq = 0;
		...
		rows.forEach(function(row) {
			...
			ibims752bVOList.push(newRow);
			$("#TB09013S_errDpchList").pqGrid("addRow", { rowData: newRow, checkEditable: false });

		});

		$("#TB09013S_errDpchList").pqGrid("refreshDataAndView");
		saveErrDpchList(ibims752bVOList);
	}



	return {
		shTB09013S: shTB09013S,
		rtTB09013S: rtTB09013S,
