const TB05040Sjs = (function () {
    ...
    $.ajax({
      ...
      success: function (data) {
        ...
        if (!isEmpty(dealInfo.cnsbDcd)) {
          $("#TB05040S_riskInspctRsltnCcd")
            .val(dealInfo.cnsbDcd)
            .prop("selected", true); // 결의협의체
        } else {
        ...
        if (!isEmpty(dealInfo.fstCnfrncF)) {
          $("#TB05040S_fstCnfrncF")
            .val(dealInfo.fstCnfrncF)
            .prop("selected", true); // 지주사전협의
        } else {
        ...
        $("#TB05040S_inspctCnfrncCcd")
          .val(dealInfo.cnsbDcd)
          .prop("selected", true); // 결의협의체
        $("#TB05040S_inspctCnfrncSqcSq").val(dealInfo.cnsbSq); // 회차정보
        $("#TB05040S_rnkNo").val(dealInfo.sn); // 순서정보

        $("#TB05040S_rsltnRsltCd")
          .val(dealInfo.rsltnRsltCd)
          .prop("selected", true); // 결의결과
  ...
  function btnReset() {
    $("#gridIbDealList").pqGrid("option", "dataModel.data", []);
    $("#gridIbDealList").pqGrid("refreshDataAndView"); // pqgrid 초기화
    $("#TB05040S_rsltnDt").val("");
    $("#TB05040S_inspctCnfrncSqcSq").val("");
    $("#TB05040S_rnkNo").val("");
    $("#TB05040S_invstCrncyCd").val("");
    $("#TB05040S_rcgAmt").val("");
    $("#TB05040S_sdnCndtF").val("");
    $("#TB05040S_etcCndtF").val("");
    $("#TB05040S_rsltCntnt").val("");
    $("#TB05040S_ibDealNo").val("");
    $("#TB05040S_ibDealNm").val("");
    $("#TB05040S_riskInspctRsltnCcd").each(function () {
      $(this).prop("selectedIndex", 0);
    });
    $("#TB05040S_fstCnfrncF").each(function () {
      $(this).prop("selectedIndex", 1);
    });
    $("#TB05040S_inspctCnfrncCcd").each(function () {
      $(this).prop("selectedIndex", 0);
    });
  }
