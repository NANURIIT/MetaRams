...
//							ddCnt2--;
			...
			// prnaCalcRstDTO.setBfBalance(process_down(inCalcDTO.getIntrSnnoPrcsDcd(),
			// bfBalance));
...
//							ddCnt2--;
		...
		do {
			...
			if (inCalcDTO.getExpDt().compareTo(DateUtil.dayAdd(lpEndDt, 1)) <= 0) {
				...
				if (lnItm.getSeq() == 1) {
					...
					// if(inCalcDTO.getExpDt().substring(0,6).compareTo(DateUtil.dayAdd(lpEndDt,1).substring(0,6))
					// == 0) {
					// //if(inCalcDTO.getExpDt().compareTo(DateUtil.dayAdd(lpEndDt, 1)) == 0){
					// loopValue = false; // 마지막LOOP처리
					// SEC_TYPE = "LST_SEC";
	...
	public String dateAdd(String baseDt, int rdmpFrqcMnum /* 상환주기개월수 */
		...
		// //baseCalendar.set(Calendar.DATE,
...
//							ddCnt2--;
	...
	public List<CalculationResultDTO> overduePrnaCalc(CalculationDTO inCalcDTO, List<CalculationResultDTO> scdhList) {
		...
		// String ovduIntrRtDcd = inCalcDTO.getOvduIntrRtDcd(); //연체이자율 구분 코드
		// (고정연체이자율/가산연체이자율)
			...
			if (scdhInfo.getPrcsCpltYn() == null || "N".equals(scdhInfo.getPrcsCpltYn())) { // 미처리
				...
				if (scdhInfo.getRdmpPrarIntr() == null) { // 원금상환계획정보
					...
					try {
						...
						// lnItm.setDdCnt(DateUtil.dateDiff(scdhInfo.getStrtDt(),
						// inCalcDTO.getStdrDt()));
						// }
	...
	public List<CalculationResultDTO> earlyRpnaCalc(CalculationDTO incaDto, List<CalculationResultDTO> scdhList) {
		...
		BigDecimal mdwyRdmpFeeRto = ibims204bMapper.getMdwyRdmpFeeRto(paramDTO).divide(new BigDecimal(100)); // 중도상환
																												// 수수료율
	...
	public BigDecimal getPrepdIntr(CalculationDTO param) {
		...
		// double dayRate =
		// ((param.getIntrDnumClcMthCd()=="4")?param.getIntrRdmpFrqcMnum():ddCnt)/actualType;
	...
	public List<CalculationResultDTO> intrCalcWhenMdwy(CalculationDTO inCalcDTO, List<CalculationResultDTO> scdhList,
		...
		for (int i = 0; i < scdhList.size(); i++) {
			...
			if (item.getScxDcd().equals("8")) { // 중도상환원금
				...
				// if(bfBalance == null){ //null들어오는 경우 다 막아야함 (어디서 null이 들어가는지 확인 필요:::
				// IBIMS403 TRGT_AMT컬럼)
	...
	public List<CalculationResultDTO> rescheduling(CalculationDTO inCalcDTO, List<CalculationResultDTO> earlyCalcList,
		...
		for (int i = 0; i < prnaList.size(); i++) {
			...
			if (prnaDTO.getRdmpTmrd().equals(earlyDTO.getRdmpTmrd())) {
				...
				if (prnaDTO.getPrarPrna().equals(earlyDTO.getPrarPrna())) {
					...
					reScdhPrnaDTO2.setBfBalance(prnaDTO.getBfBalance().subtract(earlyDTO.getPrarPrna())); // 대출잔액 (기존
																											// 대출잔액 -
																											// 중도상환 원금)
					reScdhPrnaDTO2.setPrarPrna(prnaDTO.getPrarPrna().subtract(earlyDTO.getPrarPrna())); // 상환예정원금 (기존
																										// 상환예정원금 - 중도상환
																										// 원금)
		...
		while (iterator.hasNext()) {
			...
			if (rsltDTO.getRdmpPrarIntr().compareTo(BigDecimal.ZERO) <= 0) { // 상환예정이자 0이나 음수면 무효
				...
				// if(rsltDTO.getStrtDt().compareTo(DateUtil.dayAdd(mdwyPrnaDTO.getStrtDt(),
				// -1)) >= 0 && rsltDTO.getEndDt().compareTo(mdwyPrnaDTO.getEndDt()) <= 0){
				// isWithinRange = true;
				// break;
				// }
				// }
	...
	public List<CalculationResultDTO> mdwyRdmpFeeCalc(CalculationDTO incaDto, List<CalculationResultDTO> scdhList) {
		...
		// List<CalculationResultDTO> mdwyRdmpFeeList = new ArrayList<>(); //중도상환 수수료
		// 리스트
		...
		for (int i = 0; i < scdhList.size(); i++) {
			try {
				...
				// boolean isWithinRange = (stdrDate.after(strtDate) ||
				// stdrDate.equals(strtDate)) && (stdrDate.before(endDate) ||
				// stdrDate.equals(endDate));
