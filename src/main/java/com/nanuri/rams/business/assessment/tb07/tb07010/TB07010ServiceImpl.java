...
public class TB07010ServiceImpl implements TB07010Service {
	...
	public int saveExcInfo(TB07010SVO paramData) {
		...
		if (in348BVO.getPrcsCpltYn() == null)
			in348BVO.setPrcsCpltYn("N");
		...
		if ("Y".equals(in348BVO.getPrcsCpltYn())) {
			...
			if (out203bdto != null) {
				ibims348BVO.setBusiNmcpCplTxtnYn((out203bdto.getBusiNmcpCplTxtnYn() == null) ? "N"
						: out203bdto.getBusiNmcpCplTxtnYn()); /* 사업부수수료과세여부 */
			}
		...
		// 화면이 적용되면 제거
		if (null == ibims402BDTO.getKrwTrslRt())
			ibims402BDTO.setKrwTrslRt(new BigDecimal(1));
		...
		ibims402BDTO.setPrnaRdmpFrqcMnum(
				((Objects.nonNull(ibims401bvo.getPrnaRdmpFrqcMnum())) ? 1 : ibims401bvo.getPrnaRdmpFrqcMnum())); // 원금상환주기개월수
		...
		if (null == String.valueOf(paramData.getPrnaRdmpFrqcMnum()))
			paramData.setPrnaRdmpFrqcMnum(1); // 화면 체크 제대로 되면 삭제
		...
		if (!"03".equals(paramData.getEprzCrdlPaiRdmpDcd())) {
			...
			if (ibims402BDTO.getDfrExpMnum() > 0) {
				strDfrExpDt = DateUtil.monthAdd(inCalcDTO.getExcDt(), inCalcDTO.getDfrExpMnum()).substring(0, 6)
						+ inCalcDTO.getExcDt().substring(6, 8);
			...
			for (int i = 0; i < prnaCalcDTO.size(); i++) {
				...
				// 예정원금
				if (prnaitem.getPrarPrna() == null) {
					prna403BDTO.setPrarPrna(new BigDecimal("0")); // 예정원금
				} else {
				...
				if ("02".equals(inCalcDTO.getPaiRdmpDcd())) {
					...
					// ) {
					// intr403BDTO.setRdmpTmrd(String.valueOf((Integer.parseInt(prnaitem.getRdmpTmrd()))));
					// // 상환회차
			...
			IBIMS410BDTO outExc410dto = setExc410b(lExTrsn, ibims401bvo, ibims402BDTO,
					(in348BVO.getPrcsCpltYn() == null ? in348BVO : ibims348BVO));
			...
			if ((ibims402BDTO.getPrcsIntrAmt() != null)
					&& (ibims402BDTO.getPrcsIntrAmt().compareTo(BigDecimal.ZERO) > 0)) {
		...
		inDTO.setExcSn(ibims402BDTO.getExcSn());
		inDTO.setTrCrcyCd(ibims402BDTO.getCrryCd()); /* 거래통화코드 */
		inDTO.setTrAmt(ibims402BDTO.getDealExcAmt()
				.add((ibims402BDTO.getPrcsIntrAmt() == null) ? BigDecimal.ZERO : ibims402BDTO.getPrcsIntrAmt()));
	...
	private IBIMS410BDTO setExc410b(
			long lTrsn, IBIMS401BVO param401, IBIMS402BDTO param402, IBIMS348BVO param348) {
		...
		setParam.setDealTrAmt(param402.getDealExcAmt().add((param402.getPrcsIntrAmt() == null) ? BigDecimal.ZERO
				: param402.getPrcsIntrAmt())); /* 딜거래금액 == 실행금액? */
		...
		// setParam.setCrdtGrntOvduIntAmt(getParam.getCrdtGrntOvduIntAmt()); /*
		// 신용공여연체이자금액 */
		// setParam.setCrdtGrntRcvbIntAmt(getParam.getCrdtGrntRcvbIntAmt()); /*
		// 신용공여미수이자금액 */
		...
		// setParam.setWcrcTrslTrFeeAmt(param348.getFeeRcivAmt()); /* 원화환산거래수수료금액 ==
		// 수수료수납금액 */
		...
		setParam.setTrAfLoanRmnd(param402.getDealExcAmt()
				.add((param401.getDealExcBlce() == null) ? BigDecimal.ZERO : param401.getDealExcBlce())); /* 거래이후대출잔액 */
		...
		// setParam.setTaxBillEvdcErlmDt(getParam.getTaxBillEvdcErlmDt()); /*
		// 세금계산서증빙등록일자 */
		// setParam.setTaxBillEvdcErlmSeq(getParam.getTaxBillEvdcErlmSeq()); /*
		// 세금계산서증빙등록순번 */
		...
		// setParam.setIssuBillEvdcErlmDt(getParam.getIssuBillEvdcErlmDt()); /*
		// 발행계산서증빙등록일자 */
		// setParam.setIssuBillPrcsSeq(getParam.getIssuBillPrcsSeq()); /* 발행계산서처리순번 */
		// setParam.setDfrmFeePrcaEclsYn(getParam.getDfrmFeePrcaEclsYn()); /*
		// 지급수수료원금제외여부 */
		// setParam.setDfrmFeeClmObjtAmt(getParam.getDfrmFeeClmObjtAmt()); /*
		// 지급수수료청구대상금액 */
	...
	private IBIMS410BDTO setFee410b(
		...
		// setParam.setWcrcTrslTrIntAmt(param402.getKrwTrslIntAmt()); /* 원화환산거래이자금액 ==
		// 원화환산이자금액*/
		setParam.setKrwTrslTrFeeAmt(param348.getFeeRcivAmt().multiply(param402.getKrwTrslRt()).setScale(2,
				RoundingMode.HALF_UP)); /* 원화환산거래수수료금액 == 수수료수납금액 */
		// setParam.setWcrcTrslCostAmt(getParam.getWcrcTrslCostAmt()); /* 원화환산비용금액 */
